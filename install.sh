...
for b in claude-fleet claude-here fleet-schedule fleet-send fleet-list fleet-read fleet-spawn fleet-jump; do
  ln -sf "$REPO/bin/$b" "$BIN_DIR/$b"
done
echo "✓ linked claude-fleet + helpers (here, schedule, send, list, read, spawn, jump) -> $BIN_DIR"
